...
enum msg_id_t {
  ...
  UBX_CFG_SBAS = 0x16,     // Configure navigation engine settings
...
struct msg_hdr_t {
  ...
  msg_id_t msg_id;
  bool same_kind(const msg_hdr_t &msg) const { return (msg_class == msg.msg_class) && (msg_id == msg.msg_id); }
} __attribute__((packed));
...
struct cfg_msg_t : msg_t {
  ...
  cfg_msg_t(msg_class_t m, msg_id_t i, uint8_t r) : msg_t(UBX_CFG, UBX_CFG_MSG, UBX_MSG_LEN(*this)) {
    cfg_msg_class = m;
    cfg_msg = i;
    rate = r;
  };
...
struct cfg_reset_t : msg_t {
  ...
  cfg_reset_t() : msg_t(UBX_CFG, UBX_CFG_RST, UBX_MSG_LEN(*this)) { init(); }

} __attribute__((packed));

// Configure navigation rate
enum time_ref_t { UBX_TIME_REF_UTC = 0, UBX_TIME_REF_GPS = 1 } __attribute__((packed));
...
struct cfg_rate_t : msg_t {
  ...
  cfg_rate_t(uint16_t gr, uint16_t nr, enum time_ref_t tr) : msg_t(UBX_CFG, UBX_CFG_RATE, UBX_MSG_LEN(*this)) {
    GPS_meas_rate = gr;
    nav_rate = nr;
    time_ref = tr;
  }
...
enum position_fix_t { UBX_POS_FIX_2D_ONLY = 1, UBX_POS_FIX_3D_ONLY = 2, UBX_POS_FIX_AUTO = 3 } __attribute__((packed));
...
struct cfg_nav5_t : msg_t {
  ...
  cfg_nav5_t() : msg_t(UBX_CFG, UBX_CFG_NAV5, UBX_MSG_LEN(*this)) {
    apply_word = 0xFF00;
    always_zero_1 = always_zero_2 = always_zero_3 = 0;
  }
...
struct cfg_sbas_t : msg_t {
  ...
  cfg_sbas_t() : msg_t(UBX_CFG, UBX_CFG_SBAS, UBX_MSG_LEN(*this)),
    mode{0x01}, // enabled, test mode off
    usage{0x07}, // Use SBAS GEOs, diff. corrections, integrity information
    max_sbas{3},
    scanmode2{0},
    scanmode1{0}
  {}
...
struct nav_status_t : msg_t {
  ...
  struct {
    ...
    bool _skip_ : 6;
    bool map_matching : 1;
  } __attribute__((packed)) fix_status;

  enum { PSM_ACQUISITION, PSM_TRACKING, PSM_POWER_OPTIMIZED_TRACKING, PSM_INACTIVE } power_safe : 2; // FW > v7.01
...
struct nav_svinfo_t : msg_t {
  ...
  static uint16_t size_for(uint8_t channels) { return sizeof(nav_svinfo_t) + (uint16_t)channels * sizeof(sv_t); }
